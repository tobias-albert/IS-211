...
public class ContactNet {
    ...
    public void registerConversationAlternative(Person sender, Person receiver) {
        boolean alreadyExists = false;
        Conversation c = new Conversation(sender, receiver);
        if (sender.conversations.containsKey(c)) {
            c.amount = sender.conversations.get(c).amount;
    ...
    public boolean hasDirectConnectionAlternative (Person p1, Person p2) {
        Conversation c1 = new Conversation(p1, p2);
        Conversation c2 = new Conversation(p2, p1);
